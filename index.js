...
function assignLeft(){     //it is also used to restart the game
  vx=-1;
  vy=10;
  x=1;
  y=1;
...
function changPositionOfBall(){
  vx+=x;
  vy+=y;
  const isTopDivAndBollCollide=findDistanceBetwenTopDivAndBoll(stick[0], boll);
  const isBottomDivAndBollCollide=findDistanceBetwenBottomDivAndBoll(stick[1], boll);
...
function findDistanceBetwenTopDivAndBoll(stick, boll){
  ...
  if(bollLeft+20>=stickLeftEnd && bollLeft<=stickRightEnd && stickY-bollY>=-11){
    return true;
...
function findDistanceBetwenBottomDivAndBoll(stick, boll){
  ...
  var stickRightEnd=stickLeftEnd+300;
  var bollLeft=bollrect.left;
  if(bollLeft+20>=stickLeftEnd && bollLeft<=stickRightEnd && stickY-bollY<10){
    return true;
